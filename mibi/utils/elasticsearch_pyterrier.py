...
class ElasticsearchRerank(Generic[T], Transformer):
    ...
    def _transform_query(self, res: DataFrame) -> DataFrame:
        docnos = res["docno"]
        if docnos.isna().any():
            raise RuntimeError("Cannot re-rank documents with missing docno.")
        search: Search = self.document_type.search(
            using=self.client, index=self.index)
        search = search.query(self.query_builder(res.iloc[0].to_dict()))
        search = search.filter(Terms(_id=list(set(docnos))))
        search = search.extra(size=len(res))
        search = search.extra(_source=False)

        response = search.execute()
        hits: Iterable[Hit] = response.hits.hits  # type: ignore
        scores_list: list[tuple[str, float]] = [  # type: ignore
            (hit._id, hit._score)
            for hit in hits
        ]
        scores = DataFrame({
            "docno": [docno for docno, _ in scores_list],
            "score": [score for _, score in scores_list],
        })
...
@dataclass(frozen=True)
class ElasticsearchRerankGet(Generic[T], Transformer):
    """
    Re-ranks documents and get document fields from an Elasticsearch index.
    ...
    def transform(self, topics_or_res: DataFrame) -> DataFrame:
        return (self._rerank >> self._get).transform(topics_or_res)
...
class ElasticsearchTransformer(Generic[T], Transformer):
    ...
    @cached_property
    def _rerank_get(self) -> ElasticsearchRerankGet:
        return ElasticsearchRerankGet(
            document_type=self.document_type,
            client=self.client,
            query_builder=self.query_builder,
            result_builder=self.result_builder,
    ...
    def transform(self, topics_or_res: DataFrame) -> DataFrame:
        if "docno" in topics_or_res.columns:
            return (self._rerank_get ^ self._retrieve).transform(topics_or_res)
